...
export default function AdminProperties() {
  ...
  const openEdit = (p: Property) => {
    ...
    setForm({
      ...
      social_media_type: p.social_media_type || "",
    });
    setShowForm(true);
  };
      ...
      {showForm && (
        <div className="bg-card rounded-xl p-6 shadow-card mb-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, social_media_type: e.target.value })
                }
            ...
            {form.amenities.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {form.amenities.map((a) => (
                  <span
                    key={a}
                    className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-secondary/10 text-secondary text-xs"
                  >
                    {a}
                    <button
                      type="button"
              ...
              <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-lg max-h-48 overflow-y-auto">
                ...
                {amenitiesList.length === 0 && (
                  <p className="px-3 py-2 text-sm text-muted-foreground">
                    No amenities. Add some in Amenities management.
                  </p>
                )}
              </div>
            )}
            ...
            {form.images.length > 0 && (
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 mt-3">
                {form.images.map((url) => (
                  <div
                    key={url}
                    className={`relative group rounded-lg overflow-hidden border-2 ${form.image === url ? "border-secondary" : "border-transparent"}`}
                  >
                    <img
                      src={url}
                      alt=""
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                      <button
                        ...
                        title="Set as cover"
                        className="p-1 bg-white/90 rounded-full text-secondary hover:scale-110 transition-transform"
                      >
                      ...
                      <button
                        ...
                        title="Remove"
                        className="p-1 bg-white/90 rounded-full text-destructive hover:scale-110 transition-transform"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                    {form.image === url && (
                      <span className="absolute top-1 left-1 bg-secondary text-accent-foreground text-[10px] px-1.5 py-0.5 rounded-full font-medium">
                        Cover
                      </span>
                    )}
                  </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={save}
              className="bg-secondary text-accent-foreground rounded-lg px-6 py-2 text-sm font-semibold hover:opacity-90"
            >
              {editing ? "Update" : "Create"}
            </button>
            <button
              onClick={() => setShowForm(false)}
              className="border border-border rounded-lg px-6 py-2 text-sm font-medium text-foreground hover:bg-muted"
            >
              Cancel
            </button>
          </div>
            ...
            {items.map((p) => (
              <tr key={p.id} className="border-t border-border">
                ...
                <td className="px-4 py-3 text-right">
                  ...
                  <button
                    onClick={() => remove(p.id)}
                    className="p-1.5 hover:bg-muted rounded-lg text-muted-foreground hover:text-destructive ml-1"
                  >
